...
- 首页重构第一波——新增 OmniBar 常驻输入栏：问候卡下方的胶囊输入框成为首页统一入口，文字与语音同一条管道。本地能看懂的（含日期/时间/命令信号，或短任务名）立即执行——「后天上午10点到11点 写论文」直接排块跳日期、「写周报」建任务；看不懂的长句自动转给 AI 访谈（也可点 ✨ 显式交给 AI）。⌘K 命令条保留为全局快捷键，与 OmniBar 共用同一执行器。
