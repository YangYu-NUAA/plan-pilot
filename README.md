...
- 首页重构第一波——新增 OmniBar 常驻输入栏：问候卡下方的胶囊输入框成为首页统一入口，文字与语音同一条管道。含日期/时间的规则安排立即执行——「后天上午10点到11点 写论文」直接排块跳日期；「周五 / 专注 / 主题」等命令直达；其余输入一律进入 AI 访谈对话，由访谈落成任务（也可点 ✨ 显式交给 AI）。⌘K 命令条保留为全局快捷键，与 OmniBar 共用同一执行器。
- 表单降级为「高级模式」：任务表单与时间块表单默认收起为「手动添加 / 手动排块」一行小字，需要精细控制时展开（状态存本地）。首页默认视图收敛为：问候 → 输入栏 → 统计 → 时间轴 + 任务列表。
- 首页产品化第二波——对话成为主流线：AI 访谈面板去掉底部输入表单，改为纯对话视图（空态带「开始今日规划访谈」启动按钮）；OmniBar 成为访谈唯一输入口，转发时按文本关键词首轮自动推断访谈范围（「本周」→ week、「月度」→ month、「长期」→ long，进行中的对话不被打断），转发后面板轻微闪烁引导视线。晨间问题收编为一行摘要（未填写/已保存状态 + 精力选择 +「✨ AI 晨间引导」+ 手填开关，默认收起、本地记忆）——AI 对话收集的固定安排照常自动落成 busy 块。首屏除 OmniBar 外零输入框。
...
- 拖拽体验升级：任务拖向时间轴时显示实时「幽灵块」预览——按任务估时还原长度、15 分钟吸附、压到不可用/固定块上变红提示冲突；拖到上/下边缘自动滚动。取代原来的细虚线落点指示。
