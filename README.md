...
> 会追问、帮你把大目标拆到“今天先做哪一步”的本地规划助手——不只是又一个待办清单。

待办清单能记下“要做什么”，却答不上“现在到底先干哪个、今天到底排不排得下”。计划引航换了个做法：把你的长期目标、今天的固定安排、临时冒出来的变化都说给它，它会像个规划助手一样**主动问你缺的信息**、把目标**逐层拆细**，自动避开会议、监考、通勤这些动不了的时间，最后排出一张真照着做得动的当天日程。

适合需要长期推进、但每天又总被各种事打断的人——科研、写作、项目申请、课程建设都行。本地优先：填你自己的大模型 API Key 就能用，不上传你的规划数据，也没有账号系统。
